  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(
          errorData.error?.message || "Failed to send email. Please try again.",
        );
      }
    } catch (err) {
      ...
      <div>
        <form className="flex flex-col" onSubmit={handleSubmit}>
          ...
          <div className="mt-4 h-16">
            {emailSubmitted && (
              <div
                className={`p-4 bg-green-900/20 border border-green-500/30 rounded-lg flex items-center space-x-3 h-full ${
                  isSuccessFading
                    ? "opacity-0 transition-opacity duration-[1500ms] ease-out"
                    : "opacity-100"
                }`}
                style={{
                  animation: isSuccessFading
                    ? "none"
                    : "customPulse 2s ease-in-out 2",
                }}
              >
